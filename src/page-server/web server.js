...
export default function(parameters)
{
	...
	({
			...
			assets: (url) =>
			{
				...
				if (_development_)
				{
					delete require.cache[require.resolve('../../assets/images/icon/logo-k3-150x150.png')]
				}

				// Add "favicon"
				result.icon = require('../../assets/images/icon/logo-k3-150x150.png')
			...
			preload: async (http, { request }) => {
				...
				if(Auth.isAuthenticated()){
					try{
						user = await fetchData('/users-self', 'GET')
					}
					catch (e){
						Auth.logout()
					}
